...
// human constraints
vector<VectorMapping::HumanConstraint> human_constraints;

// points selected by user during HitL
vector<Vector2f> selected_points_;
CorrectionType correction_ = CorrectionType::kUnknownCorrection;
...
struct segDistResidual {
  ...
  template <typename T> bool operator()(const T* const icm,
                                        const T* const theta,
                                        T* residual) const {
        ...
        T p1[2];
        T p2[2];
        p1[0] = icm[0] + (T(len_))*T(alpha[0]);
        p1[1] = icm[1] + (T(len_))*T(alpha[1]);
        p2[0] = icm[0] - (T(len_)/2.0)*T(alpha[0]);
        p2[1] = icm[1] - (T(len_)/2.0)*T(alpha[1]);
...
vector<Vector2f> SegFit(double* p1, double* p2, double* cm, double* data, int size) {
  vector<Vector2f> fit;
  Vector2f ep1;
  Vector2f ep2;
  double icm[2];
...
/*
  for (int i = 0; i<size; ++i) {
    problem.AddResidualBlock(
        ...
        new segDistResidual(data[2*i], data[2*i + 1], cm[0], cm[1], hy/2.0,
                       double(size))), NULL, icm, theta);
  }
*/
  for (int i = 0; i<size; ++i) {
    problem.AddResidualBlock(
...
void PublishLocation(
  ...
  vector_slam_msgs::CobotLocalizationMsg localization_msg_;
...
//     bool hc = false;
//     for (size_t h = 0; h < human_constraints.size(); ++h) {
//       for (size_t j = 0; j < human_constraints[h].x_a.size(); ++j) {
//         if (human_constraints[h].x_a[j] == int(i)) {
//           hc = true;
//         }
//       }
//       for (size_t j = 0; j < human_constraints[h].x_b.size(); ++j) {
//         if (human_constraints[h].x_b[j] == int(i)) {
//           hc = true;
//         }
//       }
//     }
...
//     }
    ...
    if (cc.size() >= poses.size()) {
      ...
      //cout << cc[i] << endl;
      //cout << max_cost << endl;
      //cout << cc[i]/max_cost << endl;
      //cout << (cc[i]/max_cost) * 255;
    }
    else {
      cost_color = 0x80;
    }
...
*/
    ...
    if (covariances_.size() > i) {
      ...
      if (true) {
        //cout << "drawing covariance" << endl;
        DrawPoseCovariance3D(poses[i].translation, single_cov, theta_var);
      }
    }


    //DrawLine(poses[i+1].translation, arr, kTrajectoryColor,
//&display_message_);
//    DrawLine(W1, arr, kTrajectoryColor, &display_message_);
 //   DrawLine(W2, arr, kTrajectoryColor, &display_message_);
...
void DisplayDebug() {
  ...
  if (!localization_.GetNodeData(
      &poses, &point_clouds, &pending_poses, &latest_pose)) {
  ...
  for (unsigned int i = clip_l; i < laser_message->ranges.size() - clip_u; ++i)
{
  ...
  if (point_cloud.size() > 1 || 1) {
    ...
    Pose2Df new_rel_pose;
    //new_rel_pose.translation = pose.translation - poses->back().translation;
    //new_rel_pose.angle = pose.angle - poses->back().angle;
    new_rel_pose.translation = relative_location;
    new_rel_pose.angle = relative_angle;
    odom_poses_.push_back(new_rel_pose);
...
bool LoadOdometryMessage(const rosbag::MessageInstance& message,
  ...
  if (kStandardizedData) {
    ...
    vector_slam_msgs::CobotOdometryMsgPtr odometry_message =
        message.instantiate<vector_slam_msgs::CobotOdometryMsg>();
    const string topic_name = message.getTopic();
...
bool LoadSetLocationMessage(const rosbag::MessageInstance& message,
                            Vector2f* global_location,
                            float* global_angle) {
  vector_slam_msgs::LocalizationMsgPtr set_location_message =
      message.instantiate<vector_slam_msgs::LocalizationMsg>();
  const string topic_name = message.getTopic();
...
void EvaluateConsistency(const vector<double>& poses,
  ...
  for (int i = 0; i < num_poses; i++) {
    const vector<Vector2f>& point_cloud = point_clouds[i];
    const Vector2f source(poses[3 * i + 0], poses[3 * i + 1]);
    const float pose_angle = poses[3 * i + 2];
  ...
  for (int i = 0; i < num_poses; i++) {
    for (int j = i+1; j < num_poses; j++) {
      const Vector2f source_a(poses[3 * i + 0], poses[3 * i + 1]);
      const Vector2f source_b(poses[3 * j + 0], poses[3 * j + 1]);
      //cout << " " << endl;
...
void SaveStfsandCovars(
  ...
  CHECK_EQ(poses.size(), covariances_.size());
  CHECK_EQ(poses.size(), point_clouds.size());
  fprintf(fid(), "%s\n", map_name.c_str());
  fprintf(fid(), "%lf\n", timestamp);
  ...
  for (size_t i = 0; i < poses.size(); ++i) {
    ...
    if (point_clouds[i].size() == 0) {
      Vector2f p = poses[i].translation;
      Vector2f n = Vector2f(cos(poses[i].angle), sin(poses[i].angle));
      point_clouds[i].push_back(p);
      normal_clouds[i].push_back(n);
    }
...
void LoadStfsandCovars(
  ...
  while (fscanf(fid(), "%f,%f,%f, %f,%f, %f,%f,"
    ...
    if (poses.size() == 0) {
      ...
      if (single_point_cloud[0] == pose.translation) { //pc empty
  ...
  if (single_point_cloud[0] == pose.translation) {
    single_point_cloud.clear();
    single_normal_cloud.clear();
    point_cloud.push_back(single_point_cloud);
    normal_cloud.push_back(single_normal_cloud);
  }
  else if (single_point_cloud.size() > 0) {
    const Rotation2Df R(-poses[poses.size()-1].angle);
    const Vector2f pose_location = -poses[poses.size()-1].translation;
    for (size_t i = 0; i < single_point_cloud.size(); i++) {
      single_point_cloud[i] = R*(single_point_cloud[i] + pose_location);
...
vector<SingleInput> LoadLogFile(const string& log_file) {
  ...
  for (int i = 0; i < num_entries; ++i) {
    if (getline(stream, full_line)) {
      ...
      cout << one_input.type_of_constraint << ", " << one_input.undone << endl;
  ...
  if (current_replay_index < int(logged_input.size())) {
    while (!log_entry_found && current_replay_index<int(logged_input.size())) {
      if (!logged_input[current_replay_index].undone) {
        ...
        prev_poses = poses;
        bool debug = false;
        int init_hc_size = human_constraints.size();
//         human_constraints = localization_.BatchLocalizeHotStart(
...
//                                             &poses,
//                                             selected_points_,
//                                             current_sim.type_of_constraint,
//                                             covariances_,
//                                             human_constraints);
        ...
        int final_hc_size = human_constraints.size();
        new_human_constraints = final_hc_size - init_hc_size;
...
void WriteOdom() {
  ScopedFile fid("Odom.txt", "w");
  //CHECK_EQ(, glob_odom_.size());
  for (size_t i = 0; i < odom_poses_.size(); ++i) {
    fprintf(fid(), "%f,%f,%f\n", odom_poses_[i].translation.x(),
                                 odom_poses_[i].translation.y(),
                                 odom_poses_[i].angle);
  }
}
...
void BatchLocalize(const string& keyframes_file,
  ...
  if (debug_level_ > 0) {
    ...
    localization_options_.EvaluateConsistency = EvaluateConsistency;
  ...
  //if (stfs_file == NULL && log_file == NULL) {
  if (bag_file != NULL) {
    vector<double> timestamps;
...
void LogActivity() {
  ...
  if (log_file == NULL) {
    num_total_constraints = human_constraints.size();
...
void AddCorrectionPoints(const vector_slam_msgs::GuiMouseClickEvent& msg) {
  ...
  if (correction_input_type != pending_correction_type_ &&
    ...
    correction_ = CorrectionType::kUnknownCorrection;
  } else {
    switch (correction_input_type) {
      case CorrectionType::kPointCorrection : {
        cout << "Points" << endl;
        if (correction_ == CorrectionType::kUnknownCorrection) {
          selected_points_.push_back(p1);
          correction_ = CorrectionType::kPointCorrection;
      ...
      case CorrectionType::kLineSegmentCorrection : {
        ...
        if (correction_ == CorrectionType::kUnknownCorrection) {
          selected_points_.push_back(p1);
          selected_points_.push_back(p2);
          correction_ = CorrectionType::kLineSegmentCorrection;
          pending_correction_type_ = CorrectionType::kUnknownCorrection;
        }
      } break;
      case CorrectionType::kCornerCorrection : {
        ...
        if (pending_correction_type_ == CorrectionType::kCornerCorrection &&
          ...
          selected_points_.push_back(p1);
          selected_points_.push_back(p2);
        } else if (correction_ == CorrectionType::kUnknownCorrection) {
          selected_points_.push_back(p1);
          selected_points_.push_back(p2);
          correction_ = CorrectionType::kCornerCorrection;
          pending_correction_type_ = CorrectionType::kUnknownCorrection;
        }
      } break;
      case CorrectionType::kColinearCorrection : {
        ...
        if (correction_ == CorrectionType::kUnknownCorrection) {
          selected_points_.push_back(p1);
          selected_points_.push_back(p2);
          correction_ = CorrectionType::kColinearCorrection;
          pending_correction_type_ = CorrectionType::kUnknownCorrection;
        }
      } break;
      case CorrectionType::kPerpendicularCorrection : {
        ...
        if (correction_ == CorrectionType::kUnknownCorrection) {
          selected_points_.push_back(p1);
          selected_points_.push_back(p2);
          correction_ = CorrectionType::kPerpendicularCorrection;
          pending_correction_type_ = CorrectionType::kUnknownCorrection;
        }
      } break;
      case CorrectionType::kParallelCorrection : {
        ...
        if (correction_ == CorrectionType::kUnknownCorrection) {
          selected_points_.push_back(p1);
          selected_points_.push_back(p2);
          correction_ = CorrectionType::kParallelCorrection;
          pending_correction_type_ = CorrectionType::kUnknownCorrection;
        }
      } break;
      default : {
...
void AutomaticEndpointAdjustment(vector<PointCloudf>* temp_point_clouds) {
  ...
  for (size_t k = 0; k < selected_points_.size()/2; k++) {
    ...
    for (size_t i=0; i<temp_point_clouds->size(); i++) {
      for (size_t j=0; j < temp_point_clouds[0][i].size(); j++) {
        double dst = DistanceToLineSegment(selected_points_[2*k],
                                           selected_points_[2*k+1],
                                           temp_point_clouds[0][i][j]);
        if (dst < threshold) {
          num_inliers++;
          inlier_set.push_back(temp_point_clouds[0][i][j]);
          cm += temp_point_clouds[0][i][j];
        }
      }
    }
...
bool MouseDragged(const vector_slam_msgs::GuiMouseClickEvent& msg) {
  const int sq_distance_moved =
      sq(msg.mouse_down.x - msg.mouse_up.x) +
      sq(msg.mouse_down.y - msg.mouse_up.y);
...
void KeyboardRequestCallback(const vector_slam_msgs::GuiKeyboardEvent& msg) {
  ...
  if (msg.keycode == 0x55) { //key code 85, 'u' for undo
    ...
    if (log_file == NULL) {
      ...
      cout << (new_human_constraints > 0) << endl;
      if (new_human_constraints > 0) {
        cout << "removing constraints" << endl;
        for (int i = 0; i < new_human_constraints; ++i) {
          human_constraints.pop_back();
        }
        new_human_constraints = 0;
      }
      ClearDrawingMessage(&display_message_);
      DisplayPoses(poses, init_point_clouds, normal_clouds, save_image_file);
    }
...
void LoopCloseCallback(const vector_slam_msgs::GuiMouseClickEvent& msg) {
  ...
  // string status_string = StringPrintf("Loop corrections: %s\n",
  //                                     (loop_corrections_on_ ? "on" : "off"));

  if (loop_corrections_on_) {
    AddCorrectionPoints(msg);
  }
  //else if (MouseDragged(msg)) {
  //  cout << "saving moved" << endl;
  //}
  else if (correction_ != CorrectionType::kUnknownCorrection) {
    ...
    if (points_verified == selected_points_.size()) {
      ...
      //for (size_t i = 0; i < selected_points_.size(); ++i) {
      //  cout << "point: " << i << endl;
      //  cout << selected_points_[i] << endl;
      //}

      DrawLine(selected_points_[0], selected_points_[1],
               0xFF000000, &display_message_);
      DrawLine(selected_points_[2], selected_points_[3],
               0xFF000000, &display_message_);

      if (correction_ != CorrectionType::kPointCorrection) {
        AutomaticEndpointAdjustment(&temp_point_clouds);
      }
      ...
      current_input.type_of_constraint = static_cast<int>(correction_);
      current_input.input_points = selected_points_;
      current_input.undone = 0;
      input_history.push_back(current_input);
      cout << "init human constraints: " << human_constraints.size() << endl;
//       human_constraints = localization_.BatchLocalizeHotStart(
//                                               localization_options_,
//                                               kMapName,
//                                               init_point_clouds,
...
//                                               &poses,
//                                               selected_points_,
//                                               static_cast<int>(correction_),
//                                               covariances_,
//                                               human_constraints);

      cout << "final human constraints: " << human_constraints.size() << endl;
      prev_covariances_ = covariances_;
      covariances_ = localization_.GetPoseCovariances();
      cout << "size of covariances_: " << covariances_.size() << endl;
